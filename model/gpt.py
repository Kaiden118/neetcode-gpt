...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, context_length: int, model_dim: int, num_blocks: int, num_heads: int):
        ...
        self.final = nn.LayerNorm(model_dim)
        self.vocab = nn.Linear(model_dim, vocab_size)
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        tokens = self.tokenembed(context) + self.posembed(torch.arange(context.shape[1]))
        trans = self.transformerblock(tokens)
        final = self.final(trans)
        output = self.vocab(final)
